...
class SubdivisionService(metaclass=Singleton):
    ...
    def chercherSubdivisionParID(self, type_subdivision: str, id: str,annee: int,  insee_dep: str=None) -> dict:
        """
        Recherche une subdivision par son type et son identifiant via le DAO.

        Args:
            type_subdivision (str): Le type de subdivision (Commune, Canton, Arrondissement, etc.).
            id (str): L'identifiant unique de la subdivision.
            insee_dep (str, optional): Le code département INSEE requis pour certains types de subdivision.

        Returns:
            dict: Les données de la subdivision trouvée.

        Raises:
            ValueError: Si les paramètres sont invalides ou si la subdivision n'est pas trouvée.
        """
        # Validation des entrées
        if not type_subdivision or not isinstance(type_subdivision, str):
            raise ValueError("Le type de subdivision doit être une chaîne non vide.")
        if not id or not isinstance(id, str) or not id.isdigit():
            raise ValueError("L'ID doit être une chaîne de caractères numériques.")
        if type_subdivision in ["Canton", "Arrondissement"] and not insee_dep:
            raise ValueError(f"Le type '{type_subdivision}' nécessite un code INSEE départemental (insee_dep).")

        # Génération du code INSEE
        if type_subdivision in ["Canton", "Arrondissement"]:
            # Utilise uniquement l'ID directement pour les cantons et arrondissements
            code_insee = id
        else:
        ...
        if type_subdivision in ["Canton", "Arrondissement"]:
            ...
            subdivision = self.subdivision_dao.find_by_code_insee(type_subdivision, code_insee, insee_dep)
        else:
            subdivision = self.subdivision_dao.find_by_code_insee(type_subdivision, code_insee)

        # Gestion des cas où la subdivision n'est pas trouvée
        if not subdivision:
            raise ValueError(
                f"Aucune subdivision trouvée pour le type '{type_subdivision}', ID '{id}', et code INSEE '{code_insee}'."
            )
    ...
    def zonage(self, type_subdivision: str, id: str, annee: int, dep = None):
        """
        Retourne le zonage d'une subdivision, avec ses subdivisions supérieures complètes.
        """
        subdivision = self.chercherSubdivisionParID(type_subdivision, id, annee)
        ...
        for superior_type, superior_column in hierarchy:
            ...
            if superior_id:
                # Si le type supérieur est un Arrondissement ou un Canton, inclure insee_dep
                if superior_type in ["Arrondissement", "Canton"]:
                    superior_data = self.subdivision_dao.find_by_code_insee(superior_type, superior_id, subdivision.get("insee_dep"))
                else:
                    superior_data = self.subdivision_dao.find_by_code_insee(superior_type, superior_id)
    ...
    def mettreAJourSubdivision(self, typeSubdivision: str, id: str, data: dict):
        """
    ...
    def _genererCodeINSEE(self, typeSubdivision: str, id: str) -> str:
        ...
        Args:
            typeSubdivision (str): Le type de subdivision (Commune, Département, Région, Canton, etc.).
            id (str): L'identifiant unique de la subdivision.
        ...
        Raises:
            ValueError: Si le type de subdivision est inconnu ou si l'ID est invalide.
        """
        # Validation de l'ID
        if not id or not isinstance(id, str) or not id.isdigit():
            raise ValueError("L'ID doit être une chaîne de caractères numériques non vide.")

        # Génération du code INSEE en fonction du type de subdivision
        if typeSubdivision == "Commune":
            return id.zfill(5)  # Le code INSEE des communes est toujours sur 5 chiffres

        elif typeSubdivision == "Departement":
            # Format département : 2 chiffres sauf pour l'Outre-Mer (3 chiffres)
            ...
            raise ValueError(f"Type de subdivision inconnu pour la génération de code INSEE : {typeSubdivision}")
