...
class FichierService:
    """Ce service doit récupérer une liste de points
    (plusieurs ou un seul point)
    et le niveau souhaité. Puis, du niveau demandé, on renvoie un fichier CSV
    qui contient la réponse du niveau demandé en utilisant le service
    ...
    def creer_points_a_partir_de_coordonnees(
        ...
        for item in liste_coordonnees2:
            ...
            # Crée un nouvel objet PointGeographique avec la latitude,
            # la longitude et le système de coordonnées
            point = PointGeographique(
                latitude=lat, longitude=lon, typecoordonnees=systeme
            )
            points.append(point)

        return points

    def reponse_niveau_demande(
        self, liste_points, type_subdivision: str, annee: int = 2024
    ) -> List[PointGeographique]:
        """Renvoie la réponse du niveau demandé pour chaque point de la
        liste donnée par l'utilisateur en utilisant la méthode
        localiserPointDansSubdivision du service localisation_service."""
        liste_reponse = []
    ...
    """def verifier_nom_subdivision_de_la_liste(
        self, liste_reponse: List[PointGeographique]
    ) -> str:
        # vérifier que la liste_reponse n'est composée que d'un même nom
        # de subdivision
        # et renvoyer le nom de subdivision
        nom_subdivision = liste_reponse[0].nom
        for point in liste_reponse:
            if point.nom != nom_subdivision:
                raise ValueError("La liste de points est composée de
                plusieurs subdivisions")
        return nom_subdivision"""

    def mettre_reponse_dans_csv(
        self, liste_points, type_subdivision: str, annee: int = 2024
    ) -> None:
        """Mettre la réponse du niveau demandé dans un fichier CSV."""
        liste_reponse = self.reponse_niveau_demande(
            liste_points, type_subdivision, annee
        )
        liste = self.creer_points_a_partir_de_coordonnees(liste_points)
        ...
        with open(
            ...
            for point, subdivision in zip(liste, liste_reponse):
                writer.writerow([point.latitude, point.longitude,
                                 point.typecoordonnees,
                                 subdivision])

        print(f"Les résultats ont été enregistrés dans 'reponse.csv' pour "
              f"la subdivision '{type_subdivision}'.")
