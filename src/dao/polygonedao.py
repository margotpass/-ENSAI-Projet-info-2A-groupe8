...
class PolygoneDAO:
    ...
    def get_polygones_from_geom(
        self, geom_coordinates: list
    ) -> List[PolygonePrimaire]:
        """
        ...
        - Les éléments suivants (le cas échéant) sont des trous
        (anneaux intérieurs)
        """
        polygones = []
        ...
        exterior_points = self.point_geographique_dao.get_points_from_geom(
            exterior_coords
        )
        ...
        for interior_coords in geom_coordinates[1:]:
            interior_points = self.point_geographique_dao.get_points_from_geom(
                interior_coords
            )
