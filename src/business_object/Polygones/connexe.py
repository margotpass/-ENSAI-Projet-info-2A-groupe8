...
class Connexe(PolygonePrimaire):
    ...
    def __init__(self, connexe=None):
        ...
        Paramètres:
            connexe (liste): liste de connexes. Défaut à None.
        ...
        if connexe is None:
            ...
            # est bien une instance de PolygonePrimaire
            if not all(
                isinstance(polygone, PolygonePrimaire)
                for polygone in connexe
            ):
                raise TypeError(
                    ("Tous les éléments doivent être des instances "
                     "de PolygonePrimaire")
                )
    ...
    def __str__(self):
        ...
        return "Connexe: [" + ', '.join(
            str(polygone) for polygone in self.connexe
        ) + "]"
    ...
    def ajouter_polygone(self, polygone):
        ...
        if isinstance(polygone, PolygonePrimaire):
            ...
            raise TypeError(
                "L'objet ajouté doit être une instance de PolygonePrimaire"
            )
