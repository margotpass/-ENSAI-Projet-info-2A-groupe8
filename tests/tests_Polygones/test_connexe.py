...
def polygones_connexes():
    """Fixture pour créer un ensemble de polygones connexes avec des polygones primaires pour les tests"""
...
def test_initialisation_connexe_avec_polygones():
    """Test pour vérifier l'initialisation d'une liste connexe avec des polygones primaires"""
    point1 = PointGeographique(48.858844, 2.294351, "WGS84")
    point2 = PointGeographique(51.507351, -0.127758, "WGS84")
    polygone1 = PolygonePrimaire([point1, point2])
...
def test_get_contour(polygones_connexes):
    """Test pour vérifier que get_connexe retourne la liste actuelle des polygones"""
    assert polygones_connexes.get_contour() == [polygones_connexes.get_contour()[0], polygones_connexes.get_contour()[1]]
